...
void waitForVBlank(void) {
  ...
  while (SCANLINECOUNTER > 160) //b/c height/ rows is 160
    ;
...
void setPixel(int row, int col, u16 color) {
  ...
  int offset = row * 240 + col; //calculate offset
  videoBuffer[offset] = color; //set that offset to said color
}
...
void drawFullScreenImageDMA(const u16 *image) {
  ...
  DMA[3].src = image;
  DMA[3].dst = &videoBuffer[OFFSET(0, 0, 240)]; //(row, col, WIDTH)
